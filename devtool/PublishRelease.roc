...
PublishRelease := [].{
	...
	validate_origin! = |repository_name| {
		origin = PublishRelease.git_output!(
			["config", "--get", "remote.origin.url"],
		)?.trim()
		fetch_urls = PublishRelease.git_output!(
			["remote", "get-url", "--all", "origin"],
		)?
			.split_on("\n")
			.keep_if(|line| !line.is_empty())
		push_urls = PublishRelease.git_output!(
	...
	validate_api = |api| {
		production =
			Url.scheme(api) == Https and
				Url.host(api) == "api.github.com" and
					Url.port(api) == None
	...
	artifact! = |root, name| {
		...
		output = Cmd.new_str("sha256sum")
			...
			.exec_output!()?
		digest = output.stdout_utf8
			.split_on(" ")
			.keep_if(|part| !part.is_empty())
			.first() ?? ""
	...
	artifacts_help! = |root, names|
		match names {
			...
			[name, .. as rest] =>
				Ok(
					[PublishRelease.artifact!(root, name)?].concat(
						PublishRelease.artifacts_help!(root, rest)?,
					),
				)
			}

	artifacts! = |root, release|
		PublishRelease.artifacts_help!(root, release.assets)
	...
	annotated_tag = |contents, expected_tag|
		match contents.split_on("\n") {
			[object_line, "type commit", tag_line, ..] =>
				if object_line.starts_with("object ") and
					tag_line == "tag ${expected_tag}" {
					target = Str.join_with(
						object_line.split_on("object ").drop_first(1),
						"object ",
					)
					if Release.is_commit_id(target) {
						Ok(
							AnnotatedTag({
								name: PublishRelease.tag_message(contents)?,
		...
		match local {
			...
			_ => {
				_ = GitHub.decide(
					release,
					expected_assets,
					{ release: NoRelease, tag: local },
				) ? TagCollision
				{}
			}
		}
		match PublishRelease.git!([
			"-c",
	...
	run! = || {
		...
		api = PublishRelease.validate_api(
			Url.parse(
				PublishRelease.required_env!("GITHUB_API_URL")?,
			) ? InvalidApiUrl,
		)?
		repository = Release.parse_repo_path(repository_name)?
		PublishRelease.validate_origin!(repository_name)?

		status = PublishRelease.git_output!([
			"status",
			"--porcelain",
			"--untracked-files=all",
		])?
		...
		release_state = GitHubApi.get_release!(
			api,
			repository,
			release.tag_name,
			token,
		) ? InspectReleaseFailed
		decision = GitHub.decide(
			release,
			expected_assets,
			{ release: release_state, tag: tag_state },
		...
		confirmed = match GitHubApi.get_release_by_id!(
			api,
			repository,
			draft.id,
			token,
		...
		if !confirmed.draft or
			!GitHub.has_published_assets(confirmed.assets, expected_assets) {
			return Err(UploadedAssetsMismatch)
		}
		PublishRelease.require_remote_tag!(release, expected_assets)?
		GitHubApi.publish_draft!(
			api,
			repository,
			confirmed.id,
			release,
