...
NixBackend :: [].{
	...
	environment_shell = |name| "kai-env-${name}"
	...
	render_selected = |ir, names, builds, workspace, staging, budget| {
		...
		if !ir.extensions.is_empty() {
			...
			return Err(
				"this Kaifile does not support extensions yet; "
					.concat("Kaifile.roc uses ${Str.join_with(kinds, ", ")}"),
			)
		}
		...
		for shell in ir.shells {
			if shell.name.starts_with("kai-env-") {
				return Err("shell prefix kai-env- is reserved")
			}
		...
		$rendered = append_rendered(
			$rendered,
			lines([
				"# Generated by kai from Kaifile.roc. Do not edit.",
				"{",
		...
		lines([
			...
			lines([
				...
				"              artifacts = pkgs.linkFarm "
					.concat(quote("kai-artifacts-${build.name}"))
...
expect match NixBackend.render(
	...
	Ok(text) => text.contains("\"kai-env-dev\" = environments.\"dev\"")
	Err(_) => False
}
...
expect NixBackend.render(
	mk({
		..simple,
		shells: [{ name: "kai-env-dev", environment: "dev" }],
	}),
).is_err()
...
expect match plan_fixture(
	...
	Ok({ steps: [plan] }) => plan.action == Run("check") and plan.argv == [
		...
		"--no-update-lock-file",
		"--no-write-lock-file",
		"path:/generated#devShells.x86_64-linux.kai-env-builder",
		"--command",
		"python3",
		"check.py",
		"configured argument",
