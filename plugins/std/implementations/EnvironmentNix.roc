# Shared glue between Kai environment configuration and the Nix backend.
import kai.Plugin
...
EnvironmentNix := [].{
	...
	collect_overlays = |entries, collected|
		match entries {
			...
			[first, .. as rest] => {
				...
				EnvironmentNix.collect_overlays(rest, EnvironmentNix.append_unseen(overlays, collected))
			}
		}
	...
	all_overlays = |context| {
		...
		Plugin.renderer_validation(Plugin.validate_string_list(overlays, NixBackend.overlay_rules))?
		Ok(overlays)
	}
	...
	render_result = |context, pkgs, overlays, unsupported_message| {
		flake = EnvironmentNix.render_flake(context, pkgs, overlays, Bool.False, unsupported_message)?
		Ok(
			Plugin.RenderResult.{
				actions: [],
