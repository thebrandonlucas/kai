...
Body := [].{
	...
	DiagnosticKind : [
		...
		UnknownField(Str),
		WrongListItem(Str),
		WrongType({ expected : ValueShape, field : Str }),
	]
	Diagnostic := { byte_offset : U64, kind : DiagnosticKind }
	...
	parse_fields : List(U8), U64, List(Field), List(Entry) -> Try({ entries : List(Entry), rest : U64 }, Diagnostic)
	parse_fields = |bytes, index, fields, entries| {
		...
		if start >= bytes.len() {
			...
			field = Body.find_field(fields, name) ? |_| { byte_offset: start, kind: UnknownField(name) }
			if Body.has_entry(entries, name) {
				Err({ byte_offset: start, kind: DuplicateField(name) })
			} else {
				colon = Body.skip_trivia(bytes, name_result.rest)
				if Body.byte_at(bytes, colon) != 58 {
					Err({ byte_offset: colon, kind: InvalidSyntax("expected ':' after field '${name}'") })
				} else {
	...
	parse_value : List(U8), U64, Field -> Try({ rest : U64, value : Value }, Diagnostic)
	parse_value = |bytes, index, field|
		match field.value {
			String =>
				if Body.byte_at(bytes, index) != 34 {
					Err({ byte_offset: index, kind: WrongType({ expected: String, field: field.name }) })
				} else {
			...
			StringList =>
				if Body.byte_at(bytes, index) != 91 {
					Err({ byte_offset: index, kind: WrongType({ expected: StringList, field: field.name }) })
				} else {
					parsed = Body.parse_string_list(bytes, index + 1, field.name, [], Bool.True)?
					Ok({ rest: parsed.rest, value: StringListValue(parsed.values) })
				}
			}
