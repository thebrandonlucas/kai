...
Plugin := [].{
	...
	append_config_descriptors = |descriptors, additions|
		match additions {
			[] => descriptors
			[first, .. as rest] => {
				updated = if descriptors.contains(first) descriptors else descriptors.append(first)
	...
	validate_config_descriptor_against = |descriptor, remaining, plugin|
		match remaining {
			[] => Ok({})
			[first, .. as rest] =>
				if descriptor.block == first.block and descriptor.kind == first.kind and descriptor.body != first.body {
