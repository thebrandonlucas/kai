...
ExternalPlugin := [].{
	...
	write_implementation = PluginApi.Implementation.{
		actions: [WriteConfigUtf8({ path: "external-plugin-output.txt" })],
		backend: file,
		command: write_command,
	}

	write : Config -> { implementation : PluginApi.Implementation, rendered_config : Str }
	write = |config| {
...
expect ExternalPlugin.write({ content: "external plugin worked\n" }).rendered_config == "external plugin worked\n"
