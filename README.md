...
Run `kai rollback production` to atomically select the generation that was active immediately before the current one. Each deployment records that predecessor explicitly, so repeated rollbacks follow activation history even after deploying from an older generation. Rollback uses the destination from the existing `deploy production` block; it does not rebuild or copy the artifact, delete any generation, or restart a process. It fails without changing `current` when no earlier generation remains. Generation pruning remains deferred.

Kai never guesses that a deployment lock is stale or breaks one automatically. If deployment or rollback was interrupted and Kai reports the lock path, first confirm that no activation for that deployment is still running as the remote SSH user. Only then remove the empty lock directory on the remote host with `rmdir -- "$HOME/.local/state/kai/deployments/production/lock"` and retry. Replace `production` with the deployment name; never remove the lock while an activation may still be using it.
